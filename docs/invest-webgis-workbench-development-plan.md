...
## 0. 当前进度与下一步排期（2026-05-27 更新）

本文件是项目唯一权威开发计划。临时性的阶段总结、验收文档和实现说明只能作为证据材料，后续任务排期以本文件为准。

### 0.1 当前已完成状态

截至 2026-05-27，项目已经完成以下阶段性工作：

- MVP 三栏 Workbench 骨架已建立：左侧 Files/Layers，中间 MapLibre 地图，右侧 Carbon 模型运行、日志和输出区域。
- 后端 FastAPI 最小骨架已建立：支持 health、assets、jobs、logs、outputs、output preview 等基础接口。
- 前端和后端已具备基础联动：前端可读取资产、创建 job、轮询日志、查看输出。
- Raster output preview 分支已验收并推送，模型输出 GeoTIFF 可通过 preview endpoint 返回 `image/png`。
- Carbon runner 已从纯 stub 升级为 `auto / real` 双模式：
  - `auto`：有 InVEST 时运行真实模型，缺少 InVEST 时显式回退 development stub。
  - `real`：强制要求 `natcap.invest`，缺失依赖或输入无效时失败并写入日志。
- `gsms-invest` conda 环境已创建并验证：
  - `natcap.invest 3.19.0`
  - `rasterio 1.4.3`
  - `fastapi 0.136.3`
- 使用 Willamette sample data 已验证真实 Carbon 运行成功，并生成真实输出：
...
- 前端 Run 面板尚未完整暴露 `run_mode=real` 选择。
- 前端 job 状态、日志和输出回流还需要整理成稳定用户路径。
- 真实 Carbon 输出已经可由后端生成和预览，但前端尚未完成“运行完成后自动加入图层”的稳定闭环。
- `backend/environment.yml` 尚未固化，当前真实 InVEST 环境依赖记录在开发规范中。
- 一键 sample Carbon demo 尚未完成，仍需要把样例资产注册、自动绑定和运行路径产品化。

### 0.3 下一阶段开发计划（明确时间点）

#### 2026-05-27：统一计划与环境基线

开发内容：

- 合并后续开发计划到本文件，确保项目只有一份权威开发计划。
- 保留阶段总结、验收文档、runner notes 作为辅助记录，不再作为任务排期来源。
- 将 conda / InVEST 注意事项写入开发规范和 Carbon runner notes。

验收标准：

- 仓库中只有 `docs/invest-webgis-workbench-development-plan.md` 作为开发计划。
- 开发规范明确记录 `gsms-invest` 环境、启动方式、版本约束和真实 Carbon 验证标准。
- 当前分支推送到远程后，工作区保持干净。

#### 2026-05-28：前端 Run 面板接入真实 Carbon

开发内容：

- 在右侧 Carbon 模型面板增加 `run_mode` 控件，支持 `auto` 和 `real`。
- 默认使用 `auto`，开发验证和演示时允许切换到 `real`。
- Run 按钮提交 `/api/jobs` 时携带 asset id、results suffix、sequestration 设置和 run mode。
- 前端对缺少 baseline LULC、carbon pools CSV 的情况给出明确错误。

验收标准：

- 在前端选择 sample Carbon 输入后，点击 Run 能创建 job。
- 使用 `run_mode=real` 时，后端日志包含 `running natcap.invest.carbon.execute`。
- 输入缺失时不会提交 job，并能在右侧面板看到明确提示。

#### 2026-05-29：Job 状态、日志轮询与错误展示

开发内容：

- 前端轮询 `/api/jobs/{job_id}` 和 `/api/jobs/{job_id}/logs`。
- 右侧显示 job 状态：`pending / validating / running / succeeded / failed`。
- LogConsole 支持自动滚动、失败高亮和复制日志。
- Run 运行中禁用重复提交。

验收标准：

- 用户不看终端也能判断 job 当前状态。
- Carbon 成功、失败、缺少依赖、输入错误均能在日志面板定位原因。
- 刷新页面后，仍可通过最近 job 信息恢复日志查看能力，至少能通过 job id 重新查询。

#### 2026-05-30：输出回流到前端 Outputs 和 Layers

开发内容：

- job 成功后前端拉取 `/api/jobs/{job_id}/outputs`。
- 右侧 OutputList 展示所有输出文件，支持下载。
- 优先识别 `c_storage_bas_*.tif` 作为主要 Carbon raster 输出。
- 成功后自动把主要 raster 输出加入 Layers，并在地图中显示 preview。
- 输出图层命名包含 job suffix 或 job id，避免多次运行混淆。

验收标准：

- Carbon 真实运行成功后，OutputList 出现真实 InVEST 输出。
- `c_storage_bas_*.tif` 自动进入 Layers。
- 地图中能显示该输出 raster preview。
- 用户可以调节输出图层透明度，并与输入 LULC 图层对比。

#### 2026-05-31：固化 conda 环境与本地启动文档

开发内容：

- 新增 `backend/environment.yml`，固化真实 InVEST 运行环境。
- 更新 README，明确：
  - 如何创建 `gsms-invest`
  - 如何启动后端
  - 如何启动前端
  - 如何运行 real Carbon demo
- 明确 `backend/requirements.txt` 与 conda 环境的职责边界。

验收标准：

- 新机器可按 README 使用 `conda env create -f backend/environment.yml` 创建环境。
- 按 README 启动后端后，`GET /api/health` 正常。
- README 中包含 real Carbon 验证路径和常见失败排查。

#### 2026-06-01：一键 Sample Carbon Demo

开发内容：

- 前端增加 `Use sample Carbon data` 入口。
- 后端提供样例资产注册或扫描接口。
- 自动绑定 baseline LULC 和 carbon pools CSV。
- 用户只需点击 sample、Run、查看输出，即可完成最短 demo。

验收标准：

- 从打开 `/workbench` 到看到 Carbon 输出图层，不需要手动移动文件或修改数据库。
- sample demo 可在 `auto` 模式跑通。
- 在已激活 `gsms-invest` 的环境下，sample demo 可在 `real` 模式跑通。

#### 2026-06-02：阶段验收与 MVP 闭环文档

开发内容：

- 编写本阶段验收记录。
- 记录真实 Carbon demo 的操作步骤、输入、输出、日志关键行和截图位置。
- 对照 MVP 完成定义更新已完成/未完成清单。
- 修复验收中发现的阻断级问题。

验收标准：

- 有一份阶段性验收文档说明 2026-05-28 至 2026-06-02 的完成情况。
- 前端三栏 workbench 可演示真实 Carbon：选择输入、运行、看日志、看输出、地图预览。
- 未完成项被明确列入后续阶段，而不是隐藏在验收结论中。

### 0.4 当前优先级

最高优先级是完成“前端运行真实 Carbon + 日志 + 输出回流地图”的闭环。SQLite、WebSocket、完整 TiTiler、更多 InVEST 模型和 agent/skills 接入都排在该闭环之后。
...
- 1366x768 下核心操作仍可完成。
- 当前阶段不要求手机端适配；低于桌面宽度的视口可保持最小宽度或横向滚动。
- 浏览器控制台无关键错误。
